...
const KIDS_FORM = ['Hello 家長你好！','家長可以填寫返以下嘅資料先！','','小朋友姓名：','歲數：','性別：M / F','跳舞經驗：Yes / No','（如有可附上影片作參考）','家長聯絡電話（WhatsApp）：','*所有資料保密只作學校內部參考','','我哋會有專業嘅導師團隊為你睇返最適合小朋友歲數以及程度的課程！','推薦返俾小朋友嚟試堂嫁！'].join('\n');
...
const KIDS_PRICE = ['Rookids Price list','','試堂','首次試堂  $120','第二次試堂  $200','','套票','四堂（35天有效）  $980','八堂（70天有效）  $1780','十二堂（105天有效）  $2380','個人特訓套票二十四堂  $3800','家庭套票二十四堂  $4000','','其他','單堂  $250'].join('\n');
...
function isLeaveTopic(text) {
  return /請假|唔舒服|生病|發燻|感冒|學校有事|缺席|唔得閒上|病假|請早退/.test(text || '');
}
...
function parseTimeRange(text) {
  ...
  t = t.replace(/([\u5341]?[\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5169])[\u9ede\u6642]/g, (_, s) => `${CNH[s] || 7}:`);
  t = t.replace(/點/g, ':').replace(/時/g, ':');
  const pm = /\bpm\b|晚|夜/.test(t);
  ...
  if (pm) return { start: '19:00', end: '20:00' };
  return null;
}
...
function roomLabel(room) {
  const id = room?.room || room || '';
  return id === 'ROOMAB' ? 'AB' : id;
}
function formatSlots(room, window) {
  ...
  if (!slots.length) return `${name}：無此時段`;
  if (window && free.length === slots.length) return `${name}：${window.start}–${window.end} 有位`;
  if (!free.length) return `${name}：${window ? `${window.start}–${window.end} 已滿` : '當日無空檔'}`;
  const groups = [];
  ...
  if (window) return `${name}：${window.start}–${window.end} 未全段可用；空檔 ${shown}`;
  return `${name}：${shown}`;
}
async function lookupAvailability(text, items, sess) {
  ...
  const footer = slotOf(items, 'availability_footer', BOOK_LINK);
  const out = [title, ...rooms.map((r) => formatSlots(r, window)), footer].join('\n');
  if (sess) {
    sess.lastDate = date;
    sess.lastRoom = room;
    sess.lastWindow = window;
    sess.lastAvail = out;
  }
  return out;
}
async function ruleAnswer(text, from, items, sess) {
  ...
  if (/改期|退款|投訴|平少少|折扣|報價|排演/.test(t)) {
    return slotOf(items, 'staff', '呢單要人手跟。正式客有 96171444。');
  }
  ...
  if (wantsAvailability(text) || AVAIL_RE.test(t)) {
    return lookupAvailability(text, items, sess);
  }
...
async function answer(text, from) {
  ...
  remember(sess, text);
  if (/staff/.test(t)) return slotOf(items, 'staff', '呢單要人手跟。正式客有 96171444。');
